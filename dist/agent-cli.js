...
import { openAgentStore, getAgentConfigDir, registerAgent, getAgent, findSessionAgent, listAgents, heartbeatAgent, stopAgent, sendAgentMessage, replyAgentMessage, agentInbox, agentOutbox, readAgentMessage, ackAgentMessage, agentThread, pollAgentMessages, waitForAgentMessages, } from './agents.js';
...
export async function runAgentCommand(args) {
    ...
    const emit = (value) => console.log(JSON.stringify(value, null, json ? 0 : 2));
    try {
        ...
        const requireValue = (value, label) => {
            if (!value?.trim())
                throw new Error(`${label} is required. See "mors agent --help".`);
            return value;
        };
        ...
        if (command === 'install') {
            if (values.runtime !== 'codex' && values.runtime !== 'claude')
                throw new Error('Install requires --runtime codex or --runtime claude.');
        ...
        if (!commands.includes(command))
            throw new Error(`Unknown agent command "${command}". See "mors agent --help".`);
        if (values.summary !== undefined && values.subject !== undefined)
            throw new Error('Use only --summary or --subject, not both.');
        if (values.ack && command !== 'reply')
            throw new Error('--ack is only valid with reply. Use the ack command for a message already handled.');
        if (values['timeout-ms'] !== undefined && command !== 'wait')
            throw new Error('--timeout-ms is only valid with wait.');
        if (values.pending && command !== 'inbox')
            throw new Error('--pending is only valid with inbox.');
        ...
        try {
            if (command === 'register') {
                const agent = registerAgent(db, { runtime: runtime, sessionId: requireValue(sessionId, '--session'),
        ...
        if (json)
            emit({ status: 'error', error: 'agent_error', message });
        else
