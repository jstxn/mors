 ...
 */
export function persistKey(keyPath, key) {
    try {
...
export function loadKey(keyPath) {
    ...
    if (mode !== KEY_FILE_MODE) {
        throw new KeyError(`Encryption key at ${keyPath} has insecure permissions (${mode.toString(8)}). Expected owner-only (${KEY_FILE_MODE.toString(8)}). Fix with: chmod 600 ${keyPath}`);
    }
    const key = readFileSync(keyPath);
    if (key.length !== KEY_LENGTH) {
        throw new KeyError(`Encryption key at ${keyPath} has invalid size (${key.length} bytes, expected ${KEY_LENGTH}).`);
    }
    return key;
}
/**
 ...
 */
export function hasSecurePermissions(keyPath) {
    if (!existsSync(keyPath))
        return false;
    const stat = statSync(keyPath);
