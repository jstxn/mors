...
function createRelayClientFromSession(configDir) {
    ...
    if (!relayBaseUrl) {
        throw new RemoteUnavailableError('Remote mode requires MORS_RELAY_BASE_URL to be set. ' +
...
function runRemoteSend(configDir, json, opts) {
    ...
    if (isNaN(recipientId)) {
        formatError('Remote send requires --to <numeric_recipient_id> (GitHub user ID)', json, 'validation_error');
        process.exitCode = 1;
        return;
    }
...
function runRemoteReply(configDir, json, opts) {
    ...
    if (isNaN(recipientId)) {
        formatError('Remote reply requires --to <numeric_recipient_id> (GitHub user ID)', json, 'validation_error');
        process.exitCode = 1;
        return;
    }
    // ── Default encrypted path: resolve key exchange session ──────────
