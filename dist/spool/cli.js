...
import { RELAY_SCOPES } from '../relay/auth-middleware.js';
import { resolveRelayBaseUrl } from '../settings.js';
import { MaildirSpool, MaildirQuotaError, MaildirSpoolError, assertMaildirName, } from './maildir.js';
import { processSpoolOnce, runSpoolBridge } from './bridge.js';
import { DEFAULT_SPOOL_POLICY, SpoolPolicyError, loadSpoolPolicy, mergeSpoolPolicy, validateSpoolCommandPolicy, } from './policy.js';
import { SpoolBridgeStateStore, defaultSpoolBridgeStatePath, } from './state.js';
import { SPOOL_SCHEMA } from './types.js';
const MAILBOXES = ['outbox', 'inbox', 'control', 'failed'];
const ZONES = ['tmp', 'new', 'cur'];
const DEFAULT_DIRECT_RELAY_SCOPES = [
    'messages:read',
    'messages:write',
    'messages:state',
    'events:read',
...
function runSpoolInit(flags, json, mode = 'spool') {
...
function runSpoolDoctor(flags, json, mode = 'spool') {
    const context = createSpoolContext(flags);
...
function runSpoolStatus(flags, json) {
    const context = createSpoolContext(flags);
    const stats = context.spool.inspect();
    ...
    if (json) {
        ...
        for (const violation of quota.violations)
            console.log(`Quota: ${violation}`);
    }
}
function runSpoolWrite(flags, json) {
    const context = createSpoolContext(flags);
    ...
    const mailbox = command.kind === 'read' || command.kind === 'ack' ? 'control' : 'outbox';
...
function runSpoolTail(flags, json) {
    const context = createSpoolContext(flags);
...
async function runSpoolWait(flags, json) {
    const context = createSpoolContext(flags);
    ...
    while (Date.now() - startedAt <= timeoutMs) {
        ...
        if (entries.length > 0) {
            if (json) {
                ...
                for (const entry of entries)
                    console.log(`inbox/new/${entry.name}`);
            }
            return;
        }
        await sleep(pollMs);
...
function runSpoolExport(flags, json) {
    const context = createSpoolContext(flags);
    const entries = MAILBOXES.flatMap((mailbox) => ['new', 'cur'].flatMap((zone) => readSpoolEntries(context.spool, mailbox, zone)));
    const payload = {
        status: 'exported',
        root: context.spool.root,
        agent_id: context.spool.agentId,
...
async function runSpoolBridgeCommand(flags, json) {
    const context = createSpoolContext(flags);
...
function runSandboxToken(flags, json) {
    ...
    if (!signingKey) {
        throw new SpoolCliError('No local signing key is available. Run "mors login" with the relay signing key configured before issuing scoped sandbox tokens.');
    }
...
function createSpoolContext(flags) {
...
function readPolicyFromFlags(flags) {
...
function readQuotaOverrides(flags) {
    return {
...
function buildSpoolWriteCommand(kind, flags) {
...
function runDoctorChecks(context) {
    const checks = [];
    const stats = context.spool.inspect();
    checks.push({
        name: 'agent_root',
...
function runWriteProbe(context) {
...
function evaluateQuotaStatus(stats) {
    const violations = [];
    if (stats.quotas.maxPendingEntries !== undefined &&
...
function readSpoolEntries(spool, mailbox, zone) {
    return spool.listEntries(mailbox, zone).map((entry) => readSpoolEntry(spool, entry));
}
function readSpoolEntry(spool, entry) {
    try {
...
function parseScopes(raw) {
    if (!raw)
        return DEFAULT_DIRECT_RELAY_SCOPES;
    const scopes = raw
        .split(',')
    ...
    if (invalid.length > 0) {
        throw new SpoolCliError(`Unsupported relay scope(s): ${invalid.join(', ')}. Allowed scopes: ${RELAY_SCOPES.join(', ')}.`);
    }
    return scopes;
}
...
function readOptionalFlag(flags, name) {
    const value = flags[name];
    if (value === undefined)
        return undefined;
    if (typeof value !== 'string' || value.trim().length === 0) {
...
function parsePositiveInteger(value, name) {
    if (value === undefined)
        return undefined;
    if (value === true)
        throw new SpoolCliError(`--${name} requires a value.`);
...
function formatSpoolError(err, json) {
    ...
    if (err instanceof SpoolPolicyError)
        error = 'policy_error';
    if (err instanceof MaildirSpoolError)
        error = 'validation_error';
...
function isRelayScope(scope) {
    return RELAY_SCOPES.includes(scope);
}
