...
export function resolveRelayStatePath(env = process.env) {
    const configured = env['MORS_RELAY_STATE_PATH']?.trim();
    return configured && configured.length > 0 ? configured : null;
}
export function loadRelayPersistenceSnapshot(statePath) {
    ...
    if (parsed['version'] !== STATE_VERSION) {
        throw new Error(`Unsupported relay persistence version in ${statePath}. Expected ${STATE_VERSION}.`);
    }
    return {
        version: STATE_VERSION,
        messageStore: parsed['messageStore'],
        accountStore: parsed['accountStore'],
        contactStore: parsed['contactStore'],
    };
}
