...
import { sendMessage, replyMessage, listInbox, readMessage, ackMessage, listThread, } from './message.js';
export function formatAgentActivity(senderName, recipientName, subject) {
...
export function agentActivity(db, message) {
...
export async function openAgentStore(configDir = getAgentConfigDir()) {
    ...
    while (!existsSync(join(configDir, '.initialized'))) {
        ...
        try {
            ...
            if (Date.now() >= deadline)
                throw new Error(`Agent store initialization is busy. If no initializer is running, remove ${lock} and retry. Existing hub files will be preserved.`, { cause: error });
            await sleep(50);
            continue;
        }
        try {
    ...
    `);
        const hasNotificationTime = () => db.pragma('table_info(agent_notifications)')
        ...
        if (!hasNotificationTime()) {
            ...
            db.transaction(() => {
                if (!hasNotificationTime())
                    db.exec('ALTER TABLE agent_notifications ADD COLUMN notified_at INTEGER NOT NULL DEFAULT 0');
            }).immediate();
        }
        return db;
    }
...
export function registerAgent(db, options) {
    if (!['codex', 'claude', 'other'].includes(options.runtime))
        throw new Error('Unknown agent runtime.');
    ...
    if (options.role !== undefined)
        requiredText(options.role, 'Role', 500);
    if (options.project !== undefined)
        requiredText(options.project, 'Project', 4096);
    return db.transaction(() => {
        const existing = db.prepare('SELECT * FROM agents WHERE runtime = ? AND session_id = ?')
            .get(options.runtime, options.sessionId);
        ...
        const named = db.prepare('SELECT id FROM agents WHERE name = ?').get(name);
        if (named && named.id !== id)
            throw new Error(`Agent name "${name}" belongs to another session. Choose a different name.`);
      ...
      ON CONFLICT(id) DO UPDATE SET name=excluded.name, project=excluded.project,
        role=excluded.role, last_seen=excluded.last_seen, stopped_at=NULL`)
            .run(id, name, options.runtime, options.sessionId, options.project ? resolve(options.project) : existing?.project ?? process.cwd(), options.role ?? existing?.role ?? '', new Date().toISOString());
        return getAgent(db, id);
    }).immediate();
}
export function getAgent(db, reference) {
    requiredText(reference, 'Agent ID or name', 512);
    const agent = db.prepare('SELECT * FROM agents WHERE id = ? OR name = ?').get(reference, reference);
    if (!agent)
        throw new Error(`Unknown agent "${reference}". Use "mors agent list --all --json".`);
    return agent;
}
export function findSessionAgent(db, runtime, sessionId) {
    const agent = db.prepare('SELECT * FROM agents WHERE runtime = ? AND session_id = ?').get(runtime, sessionId);
    if (!agent)
        throw new Error('This session is not registered. Run "mors agent register" or use the --agent address supplied by the session hook.');
    return agent;
}
export function listAgents(db, options = {}) {
    const maxAge = options.maxAgeMs ?? 15 * 60_000;
    if (!Number.isFinite(maxAge) || maxAge <= 0)
        throw new Error('Agent maximum age must be positive.');
...
function storedMessageActivity(db, messageId) {
    const message = db.prepare('SELECT sender, recipient, subject FROM messages WHERE id = ?').get(messageId);
    if (!message)
        throw new Error(`Message ${messageId} was not found after saving.`);
    return agentActivity(db, message);
}
export function sendAgentMessage(db, agentId, options) {
...
function visibleMessage(db, agentId, messageId, recipientOnly = false) {
...
export function replyAgentMessage(db, agentId, parentId, options) {
    const sender = getAgent(db, agentId).id;
...
export function agentOutbox(db, agentId) {
    ...
    FROM messages m WHERE m.sender = ? ORDER BY m.created_at DESC, m.id`)
        .all(getAgent(db, agentId).id);
}
export function readAgentMessage(db, agentId, messageId) {
...
export function pollAgentMessages(db, agentId, limit = 10) {
    ...
    return db.transaction(() => {
      ...
      ORDER BY n.message_id IS NOT NULL, n.notified_at, m.created_at, m.id LIMIT ?`)
            .all(actor, actor, now - 60_000, limit);
...
export async function waitForAgentMessages(db, agentId, options = {}) {
    ...
    if (!Number.isInteger(limit) || limit < 1 || limit > 100)
        throw new Error('Wait limit must be between 1 and 100.');
    const actor = getAgent(db, agentId).id;
    const deadline = performance.now() + timeoutMs;
    const pending = db.prepare(`SELECT * FROM messages WHERE recipient = ? AND state != 'acked'
    ORDER BY created_at, id LIMIT ?`);
    for (;;) {
        const messages = pending.all(actor, limit);
