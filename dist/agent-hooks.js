...
function runtimeValue(runtime) {
    if (runtime !== 'codex' && runtime !== 'claude')
        throw new MorsError('Hook runtime must be codex or claude.');
    return runtime;
}
function shellQuote(value) {
    return "'" + value.replace(/'/g, "'\\''") + "'";
}
function commandPrefix(hubDir) {
...
export async function handleAgentHook(runtime, input) {
    runtimeValue(runtime);
    if (!object(input))
        throw new MorsError('Hook input must be a JSON object.');
    const event = requireText(input['hook_event_name'], 'hook_event_name');
    if (!EVENTS.includes(event))
        throw new MorsError(`Unsupported Mors hook event: ${event}.`);
    ...
    if (event.startsWith('Subagent') && !childId)
        throw new MorsError(`Hook ${event} requires agent_id.`);
    const project = requireText(input['cwd'], 'cwd', 4096);
    if (!isAbsolute(project))
        throw new MorsError('Hook cwd must be an absolute path.');
...
export async function runAgentHook(runtime) {
    ...
    for await (const chunk of process.stdin) {
        ...
        size += buffer.length;
        if (size > 8 * 1024 * 1024)
            throw new MorsError('Hook input exceeds 8 MiB.');
        chunks.push(buffer);
    }
    let input;
    try {
...
export function installAgentIntegration(options) {
    ...
    for (const event of EVENTS) {
        ...
        const retained = groups.flatMap((group) => {
            const handlers = group['hooks'].filter((handler) => !object(handler) || typeof handler['command'] !== 'string' || !handler['command'].endsWith(HOOK_MARKER));
            return handlers.length || group['hooks'].length === 0 ? [{ ...group, hooks: handlers }] : [];
        });
