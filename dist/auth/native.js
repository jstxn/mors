...
export class InvalidInviteTokenError extends MorsError {
    constructor(detail) {
        super('Invalid or missing invite token. ' +
...
export class DeviceKeyNotBootstrappedError extends MorsError {
    constructor() {
        super('Device encryption keys have not been bootstrapped. ' +
...
export class NativeAuthPrerequisiteError extends MorsError {
    ...
    constructor(missing) {
        ...
        super(`Missing required authentication prerequisites: ${list}. ` +
...
export function validateInviteToken(token) {
    ...
    if (!INVITE_TOKEN_PATTERN.test(trimmed)) {
        return {
            valid: false,
            accountId: '',
            reason: 'Invite token format is invalid. Expected format: mors-invite-<hex> (at least 32 hex characters).',
        };
    }
...
export function generateSessionToken(options) {
    ...
    const ttlSeconds = options.expiresInSeconds !== undefined && options.expiresInSeconds > 0
 ...
 */
export function verifySessionToken(token, signingKey, options = {}) {
