...
import { loadSession, loadSigningKey, isAuthEnabled } from './session.js';
// ── Error types ──────────────────────────────────────────────────────
/**
...
export class NotAuthenticatedError extends MorsError {
    constructor(message) {
        super(message ?? 'Not authenticated. Run "mors login" to authenticate before using this command.');
 ...
 */
export function requireAuth(configDir) {
 ...
 */
export async function verifyTokenLiveness(accessToken, options) {
    ...
    if (!payload) {
        throw new TokenLivenessError('Session token signature is invalid — token may be expired or revoked');
    }
    return {
