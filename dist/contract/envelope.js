...
import { isValidDeliveryState } from './states.js';
...
export function validateEnvelope(envelope) {
    ...
    if (!isValidPrefixedId(envelope.id, 'message')) {
        throw new ContractValidationError('Envelope field "id" must be a non-empty string with "msg_" prefix.');
    }
    if (!isValidPrefixedId(envelope.thread_id, 'thread')) {
        throw new ContractValidationError('Envelope field "thread_id" must be a non-empty string with "thr_" prefix.');
    }
    ...
    if (!isValidOptionalId(envelope.in_reply_to)) {
        throw new ContractValidationError('Envelope field "in_reply_to" must be null or a non-empty string.');
    }
    if (envelope.in_reply_to !== null && !isValidPrefixedId(envelope.in_reply_to, 'message')) {
        throw new ContractValidationError('Envelope field "in_reply_to" must have "msg_" prefix when set.');
    }
    if (!isValidOptionalId(envelope.dedupe_key)) {
        throw new ContractValidationError('Envelope field "dedupe_key" must be null or a non-empty string.');
    }
    if (envelope.dedupe_key !== null && !isValidPrefixedId(envelope.dedupe_key, 'dedupe')) {
        throw new ContractValidationError('Envelope field "dedupe_key" must have "dup_" prefix when set.');
    }
    if (!isValidOptionalId(envelope.trace_id)) {
        throw new ContractValidationError('Envelope field "trace_id" must be null or a non-empty string.');
    }
    if (envelope.trace_id !== null && !isValidPrefixedId(envelope.trace_id, 'trace')) {
        throw new ContractValidationError('Envelope field "trace_id" must have "trc_" prefix when set.');
    }
    // State must be a valid delivery state.
    if (!isValidDeliveryState(envelope.state)) {
        throw new ContractValidationError(`Envelope field "state" must be one of: queued, delivered, acked, failed. Got: "${String(envelope.state)}".`);
    }
    ...
    if (!isValidOptionalId(envelope.subject)) {
        throw new ContractValidationError('Envelope field "subject" must be null or a non-empty string.');
    }
    if (!isValidOptionalId(envelope.read_at)) {
        throw new ContractValidationError('Envelope field "read_at" must be null or a non-empty string.');
    }
}
/**
 ...
 */
export function validateEnvelopeForSend(envelope) {
    validateEnvelope(envelope);
    if (envelope.state !== 'queued') {
        throw new ContractValidationError(`Send envelope must have state "queued". Got: "${envelope.state}".`);
    }
    if (envelope.read_at !== null) {
        throw new ContractValidationError('Send envelope must not have read_at set (messages are unread on send).');
    }
}
/**
 ...
 */
export function validateEnvelopeForReply(envelope) {
    validateEnvelopeForSend(envelope);
    if (envelope.in_reply_to === null) {
        throw new ContractValidationError('Reply envelope must have "in_reply_to" set to the parent message ID.');
    }
}
/**
...
export function validateMessageId(id) {
    if (!isValidPrefixedId(id, 'message')) {
        throw new ContractValidationError(`Invalid message ID: expected a non-empty "msg_"-prefixed string, got ${id === null ? 'null' : typeof id === 'string' ? `"${id}"` : typeof id}.`);
    }
}
