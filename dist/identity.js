 ...
 */
export function persistIdentity(configDir, identity) {
...
export function loadIdentity(configDir) {
    ...
    if (!existsSync(metadataPath)) {
        throw new KeyError(`Identity metadata not found at ${metadataPath}. Run "mors init" to create one.`);
    }
    if (!existsSync(privateKeyPath)) {
        throw new KeyError(`Identity private key not found at ${privateKeyPath}. Run "mors init" to create one.`);
    }
    // Check private key permissions.
    const stat = statSync(privateKeyPath);
    const mode = stat.mode & 0o777;
    if (mode !== PRIVATE_KEY_MODE) {
        throw new KeyError(`Identity private key at ${privateKeyPath} has insecure permissions (${mode.toString(8)}). ` +
            `Expected owner-only (${PRIVATE_KEY_MODE.toString(8)}). Fix with: chmod 600 ${privateKeyPath}`);
    }
    const privateKey = readFileSync(privateKeyPath);
    if (privateKey.length !== 32) {
        throw new KeyError(`Identity private key at ${privateKeyPath} has invalid size (${privateKey.length} bytes, expected 32).`);
    }
    ...
    try {
        ...
        throw new KeyError(`Failed to parse identity metadata at ${metadataPath}. File may be corrupted.`);
    }
    if (!metadata.publicKey || !metadata.fingerprint) {
        throw new KeyError(`Invalid identity metadata at ${metadataPath}. Missing publicKey or fingerprint.`);
    }
    const publicKey = Buffer.from(metadata.publicKey, 'hex');
    if (publicKey.length !== 32) {
        throw new KeyError(`Invalid public key in identity metadata (${publicKey.length} bytes, expected 32).`);
    }
 ...
 */
export function isInitialized(configDir) {
    return (existsSync(join(configDir, 'identity.json')) && existsSync(join(configDir, 'identity.key')));
}
/**
