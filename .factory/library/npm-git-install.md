...
2. **Conditional prepare script**: `"prepare": "if test -x node_modules/.bin/tsc; then npm run build; fi"` — skips build when `tsc` is unavailable (GitHub install path) and runs build with full error propagation when it is available (local dev path). Real build failures are NOT swallowed.
