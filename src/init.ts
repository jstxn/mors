...
export async function initCommand(options?: {
  ...
  const expectedDirs: string[] = [e2eeKeysDir];
 ...
 */
export function requireInit(configDir?: string): string {
  const dir = configDir ?? getConfigDir();
  const sentinelPath = join(dir, INIT_SENTINEL);

  if (!existsSync(sentinelPath) || !isInitialized(dir)) {
    throw new NotInitializedError(
      'mors is not initialized. Run "mors init" to set up identity and encrypted store.'
    );
  }
 ...
 */
export function getDbKeyPath(configDir: string): string {
...
function cleanupArtifacts(artifacts: string[]): void {
