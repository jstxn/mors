...
export function runDeployPreflight(
  ...
  if (!flyctlPath) {
    issues.push({
      ...
      remediation:
        'Install flyctl: brew install flyctl (macOS) or curl -L https://fly.io/install.sh | sh. ' +
  ...
  if (!hasToken) {
    ...
    if (!authOk) {
      issues.push({
        ...
        remediation:
          'Authenticate with Fly.io: run `flyctl auth login` interactively, ' +
  ...
  const appName = env['FLY_APP_NAME'] as string;
  const org = env['FLY_ORG'] as string;
  const resolvedPath = flyctlPath as string;
 ...
 */
function resolveFlyctl(env: Record<string, string | undefined>): string | undefined {
 ...
 */
function checkFlyctlAuth(flyctlPath: string): boolean {
  try {
 ...
 */
export function formatDeployIssues(issues: DeployIssue[]): string {
 ...
 */
export function formatDeployResultJson(
