 ...
 */
function isWellFormedMorsSessionToken(token: string): boolean {
  ...
  if (!principal) {
    ...
    if (isWellFormedMorsSessionToken(token)) {
      return {
        ...
        error: 'unauthorized',
        detail:
          'Token signature mismatch — the session token appears valid but was signed with a different key. ' +
