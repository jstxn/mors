...
function generateHostedAccountId(): string {
...
function serializeDeviceBundle(bundle: PublishedDeviceBundle) {
  return {
...
function getPrimaryPublishedDeviceBundle(
  accountStore: AccountStore,
  accountId: string
): PublishedDeviceBundle | null {
    ...
    skills: [
      {
        ...
        description:
          'Send and receive asynchronous messages with threading, read/ack state tracking, and deduplication.',
      ...
      },
      {
        ...
        description:
          'Subscribe to real-time message lifecycle events via Server-Sent Events with cursor-based reconnect.',
    ...
    securitySchemes: {
      mors_bearer: {
        httpAuthSecurityScheme: {
          ...
          description:
            'mors native HMAC-signed session token. Obtain via "mors login" with an invite token.',
        },
      },
    },
...
function handleAgentCard(
  ...
  if (!handleParam || handleParam.trim().length === 0) {
    const card = buildAgentCard(
      'mors-relay',
      'mors relay service — CLI-first encrypted messaging with async delivery, threaded replies, SSE streaming, and 1:1 E2EE.',
      baseUrl
    );
  ...
  const card = buildAgentCard(
    profile.handle,
    `${profile.displayName} — mors agent on ${profile.handle}. Supports async messaging, threaded replies, SSE streaming, and 1:1 E2EE.`,
    baseUrl
  );
...
export function createRelayServer(config: RelayConfig, options?: RelayServerOptions): RelayServer {
  ...
  const sseRevalidationTimers = new Set<ReturnType<typeof setInterval>>();
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (url === '/events' || url.startsWith('/events?')) {
      ...
      if (!requireScope(res, principal, 'events:read')) return;
            ...
            if (!revalidated) {
              ...
              writeSSE(res, {
                ...
                data: JSON.stringify({
                  error: 'token_expired',
                  detail:
                    'Your authentication token has expired or been revoked. Run "mors login" to re-authenticate.',
    ...
    if (url === '/accounts/register' && method === 'POST' && accountStore) {
      if (!requireScope(res, principal, 'accounts:write')) return;
      const body = await readJsonBody(req);
      if (!body) {
        sendJson(res, 400, { error: 'invalid_body', detail: 'Request body must be valid JSON.' });
        return;
    ...
    if (url === '/accounts/me' && method === 'GET' && accountStore) {
      if (!requireScope(res, principal, 'accounts:read')) return;
      const profile = accountStore.getByAccountId(principal.accountId);
      if (!profile) {
        sendJson(res, 404, {
          error: 'not_onboarded',
          detail:
            'Account has not completed onboarding. Run "mors onboard" to register your handle and profile.',
        });
        return;
      }
    ...
    if (url === '/accounts/me/device-bundle' && method === 'PUT' && accountStore) {
      ...
      if (deviceId !== principal.deviceId) {
        send403(
          res,
          'Device identity mismatch. device_id must match the authenticated device.'
        );
        return;
      }
    ...
    if (url === '/contacts/status' && method === 'POST' && contactStore) {
      if (!requireScope(res, principal, 'contacts:read')) return;
      const body = await readJsonBody(req);
      if (!body) {
        sendJson(res, 400, { error: 'invalid_body', detail: 'Request body must be valid JSON.' });
        return;
    ...
    if (url === '/contacts/add' && method === 'POST' && contactStore && accountStore) {
      if (!requireScope(res, principal, 'contacts:write')) return;
      const body = await readJsonBody(req);
      if (!body) {
        sendJson(res, 400, { error: 'invalid_body', detail: 'Request body must be valid JSON.' });
        return;
    ...
    if (url === '/messages' && method === 'POST' && messageStore) {
      if (!requireScope(res, principal, 'messages:write')) return;
      const body = await readJsonBody(req);
      if (!body) {
        sendJson(res, 400, { error: 'invalid_body', detail: 'Request body must be valid JSON.' });
        return;
    ...
    if ((url === '/inbox' || url.startsWith('/inbox?')) && method === 'GET' && messageStore) {
      if (!requireScope(res, principal, 'messages:read')) return;
      const urlObj = new URL(url, `http://localhost`);
    ...
    if (messageStore) {
      ...
      if (msgRoute) {
        ...
        if (action === 'read') {
          ...
          if (!requireScope(res, principal, 'messages:state')) return;
          try {
