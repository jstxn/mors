 ...
 */
export function verifySqlCipherAvailable(simulateUnavailable = false): void {
  if (simulateUnavailable) {
    throw new SqlCipherUnavailableError(
      'SQLCipher is not available. Install SQLCipher and rebuild native modules. ' +
  ...
  try {
    ...
    throw new SqlCipherUnavailableError(
      `SQLCipher preflight check failed: ${msg}. ` +
 ...
 */
export function openEncryptedDb(options: StoreOptions): BetterSqlite3.Database {
  const { dbPath, key } = options;

  if (!key || key.length !== 32) {
    throw new KeyError(
      `Invalid encryption key: expected 32-byte Buffer, got ${key ? `${key.length}-byte` : 'null'}.`
    );
  }
  ...
  try {
    ...
    if (
      ...
      throw new StoreEncryptionError(
        `Failed to decrypt database at ${dbPath}: wrong encryption key or corrupted database. ` +
