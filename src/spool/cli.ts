    ...
    if (subcommand === 'bridge') {
      ...
      if (once) {
        ...
        if (json) {
          ...
          console.log(
            `Processed ${result.processed} spool entr${
              result.processed === 1 ? 'y' : 'ies'
            }; sent=${result.sent}, queued=${result.queued}, read=${result.read}, acked=${result.acked}, materialized=${result.materialized}, failed=${result.failed}, deferred=${result.deferred}`
          );
        }
        return;
      }
...
function createSpoolFromFlags(flags: Record<string, string | true>): MaildirSpool {
