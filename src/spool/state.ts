...
export class SpoolBridgeStateStore {
  ...
  constructor(options: { path: string; agentId: string }) {
  ...
  load(): SpoolBridgeState | null {
    ...
    try {
      ...
      if (parsed['schema'] !== SPOOL_BRIDGE_STATE_SCHEMA) return null;
      if (parsed['agent_id'] !== this.agentId) return null;
      return parsed as unknown as SpoolBridgeState;
    } catch {
      return null;
    }
  }

  recordResult(result: SpoolBridgeResult, options: { nextRetryAt?: string } = {}): void {
  ...
  recordError(err: unknown, options: { nextRetryAt?: string } = {}): void {
    const previous = this.load();
    this.save({
      schema: SPOOL_BRIDGE_STATE_SCHEMA,
      agent_id: this.agentId,
  ...
  private save(state: SpoolBridgeState): void {
...
export function defaultSpoolBridgeStatePath(spool: MaildirSpool): string {
