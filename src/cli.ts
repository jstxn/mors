...
async function runStatus(_args: string[]): Promise<void> {
  ...
  if (!envSigningKey && !localSigningKey) {
    ...
    try {
      const principal = await verifyHostedSessionWithRelay(
        relayBaseUrl,
        session.accessToken,
        session.deviceId
      );
      ...
      const message =
        `Could not verify this session with relay ${relayBaseUrl}. ${detail} ` +
...
async function verifyHostedSessionWithRelay(
  relayBaseUrl: string,
  token: string,
    ...
    const req = doRequest(
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          if (statusCode === 401) {
            reject(
              new HostedStatusUnauthorizedError(
          ...
          if (statusCode === 404 && body['error'] === 'not_onboarded') {
            reject(
              new HostedStatusRelayProfileMissingError(
                typeof body['detail'] === 'string' ? body['detail'] : undefined
              )
            );
            return;
...
/**
 * Step result from a quickstart lifecycle step.
 */
...
Options:
  ...
  try {
    ...
    try {
      ...
      const readResult = readMessage(db, messageId as string);
      if (!readResult) {
  ...
  try {
    ...
    try {
      // messageId is guaranteed defined after successful send step
      const ackResult = ackMessage(db, messageId as string);
...
/**
 * Individual check result from a doctor diagnostic.
 */
...
Options:
  ...
  if (json) {
    ...
    if (hasFail) {
      console.log(
        `Result: UNHEALTHY (${checks.filter((c) => c.status === 'fail').length} failing check(s))`
      );
    } else {
