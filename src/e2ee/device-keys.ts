 ...
 */
export function generateDeviceKeys(): DeviceKeyBundle {
  ...
  const x25519PublicKey = Buffer.from(
    x25519Pair.publicKey.subarray(x25519Pair.publicKey.length - KEY_SIZE)
  );
  ...
  const x25519PrivateKey = Buffer.from(
    x25519Pair.privateKey.subarray(x25519Pair.privateKey.length - KEY_SIZE)
  );
  ...
  const ed25519PublicKey = Buffer.from(
    ed25519Pair.publicKey.subarray(ed25519Pair.publicKey.length - KEY_SIZE)
  );

  // Extract raw 32-byte Ed25519 private key seed from DER-encoded PKCS#8
  const ed25519PrivateKey = Buffer.from(
    ed25519Pair.privateKey.subarray(ed25519Pair.privateKey.length - KEY_SIZE)
  );
 ...
 */
export function persistDeviceKeys(keysDir: string, bundle: DeviceKeyBundle): void {
  try {
    ...
    if (err instanceof DeviceKeyError) throw err;
 ...
 */
export function loadDeviceKeys(keysDir: string): DeviceKeyBundle {
  const metadataPath = join(keysDir, METADATA_FILE);
  const x25519Path = join(keysDir, X25519_KEY_FILE);
  const ed25519Path = join(keysDir, ED25519_KEY_FILE);
  ...
  if (!existsSync(metadataPath)) {
    throw new DeviceKeyError(
      `Device key metadata not found at ${metadataPath}. ` +
  ...
  if (!existsSync(x25519Path)) {
    throw new DeviceKeyError(
      `X25519 private key not found at ${x25519Path}. ` +
  ...
  if (!existsSync(ed25519Path)) {
    throw new DeviceKeyError(
      `Ed25519 private key not found at ${ed25519Path}. ` +
        'Device keys may be corrupted. Run "mors init" to re-bootstrap.'
    );
  }
  ...
  if (x25519Mode !== PRIVATE_KEY_MODE) {
    throw new DeviceKeyError(
      `X25519 private key at ${x25519Path} has insecure permissions (${x25519Mode.toString(8)}). ` +
  ...
  if (ed25519Mode !== PRIVATE_KEY_MODE) {
    throw new DeviceKeyError(
      `Ed25519 private key at ${ed25519Path} has insecure permissions (${ed25519Mode.toString(8)}). ` +
  ...
  if (x25519PrivateKey.length !== KEY_SIZE) {
    throw new DeviceKeyError(
      `X25519 private key has invalid size (${x25519PrivateKey.length} bytes, expected ${KEY_SIZE}).`
    );
  }
  ...
  if (ed25519PrivateKey.length !== KEY_SIZE) {
    throw new DeviceKeyError(
      `Ed25519 private key has invalid size (${ed25519PrivateKey.length} bytes, expected ${KEY_SIZE}).`
    );
  }

  // Parse metadata
  let metadata: DeviceKeyMetadata;
  try {
    const raw = readFileSync(metadataPath, 'utf-8');
    metadata = JSON.parse(raw) as DeviceKeyMetadata;
  } catch {
    throw new DeviceKeyError(
      `Failed to parse device key metadata at ${metadataPath}. File may be corrupted.`
    );
  }
  ...
  if (!metadata.deviceId || !metadata.fingerprint || !metadata.x25519PublicKey || !metadata.ed25519PublicKey) {
    throw new DeviceKeyError(
      `Invalid device key metadata at ${metadataPath}. Missing required fields.`
    );
  }
  ...
  if (x25519PublicKey.length !== KEY_SIZE) {
    throw new DeviceKeyError(
      `Invalid X25519 public key in metadata (${x25519PublicKey.length} bytes, expected ${KEY_SIZE}).`
    );
  }

  if (ed25519PublicKey.length !== KEY_SIZE) {
    throw new DeviceKeyError(
      `Invalid Ed25519 public key in metadata (${ed25519PublicKey.length} bytes, expected ${KEY_SIZE}).`
    );
  }
  ...
  if (metadata.fingerprint !== expectedFingerprint) {
    throw new DeviceKeyError(
      'Device key fingerprint mismatch. Key files may be corrupted or tampered with. ' +
 ...
 */
export function isDeviceBootstrapped(keysDir: string): boolean {
  return (
    existsSync(join(keysDir, METADATA_FILE)) &&
 ...
 */
export function getDeviceKeysDir(configDir: string): string {
