...
import { isDeviceBootstrapped, loadDeviceKeys, type DeviceKeyBundle } from './device-keys.js';
import { DeviceNotBootstrappedError } from '../errors.js';
 ...
 */
export function requireDeviceBootstrap(keysDir: string): DeviceKeyBundle {
  if (!isDeviceBootstrapped(keysDir)) {
    throw new DeviceNotBootstrappedError();
  }

  return loadDeviceKeys(keysDir);
}
 ...
 * @throws DeviceNotBootstrappedError if keys are not bootstrapped.
 */
export function assertDeviceBootstrapped(keysDir: string): void {
  if (!isDeviceBootstrapped(keysDir)) {
    throw new DeviceNotBootstrappedError();
  }
}
