 ...
 */
function validateSharedSecret(sharedSecret: Buffer): void {
  if (!Buffer.isBuffer(sharedSecret) || sharedSecret.length !== SHARED_SECRET_SIZE) {
    throw new CipherError(
      `Invalid shared secret: expected ${SHARED_SECRET_SIZE} bytes, ` +
 ...
 */
export function encryptMessage(sharedSecret: Buffer, plaintext: string): EncryptedPayload {
 ...
 */
export function decryptMessage(sharedSecret: Buffer, payload: EncryptedPayload): string {
  validateSharedSecret(sharedSecret);
  ...
  try {
    ...
    if (err instanceof CipherError) throw err;
    throw new CipherError(
      'Decryption failed: message integrity or authentication check failed. ' +
 ...
 */
export function decryptMessageStrict(sharedSecret: Buffer, payload: EncryptedPayload): string {
  validateSharedSecret(sharedSecret);
  validatePayload(payload);
